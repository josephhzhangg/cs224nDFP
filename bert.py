...
class BertSelfAttention(nn.Module):
    ...
    def attention(self, key, query, value, attention_mask):
        ...
        # bs = query.size(0) I am not sure about how to find bs and seq_len from here
        # bs, seq_len = key.shape[:2] this is probably wrong

        att_scores = torch.matmul(
            query, key.transpose(-2, -1)) * (1.0 / math.sqrt(key.size(-1)))
        # my guess from the instructions
        bs, seq_len = att_scores.shape[0], att_scores.shape[2]
        att_scores = att_scores.masked_fill_(
            attention_mask != 0, float('-inf'))
        att_scores = F.softmax(att_scores, dim=-1)
        V_prime = torch.matmul(att_scores, value)
        V_prime = V_prime.transpose(1, 2).contiguous().view(
            bs, seq_len, self.num_attention_heads * self.attention_head_size)
        # to concat multi-heads ands recover the original shape, perhaps .view(bs, seq_len, self.num_attention_heads * self.attention_head_size?)
        return V_prime
...
class BertLayer(nn.Module):
    ...
    def add_norm(self, input, output, dense_layer, dropout, ln_layer):
        ...
        # attempt1: joseph
        """
    dropOutValue = dropout(output)
    
    x = input + dropOutValue
    ret = ln_layer(x + dense_layer(x))
    return ret
    """
        # attempt2: joseph
        ...
        # print(output.size())
        # dropOutValue = dropout(output)
        output = dense_layer(output)  # could be switched with above
        output = dropout(output)

        x = input + output
        # print("new output size: " + str(output.size()))
        ret = ln_layer(x)
        return ret
    ...
    def forward(self, hidden_states, attention_mask):
        ...
        # 2
        add_norm_output = self.add_norm(hidden_states, attention_output, self.attention_dense,
                                        self.attention_dropout, self.attention_layer_norm)  # fixed bug: alex
        # 3
        feed_forward_output = self.interm_dense(add_norm_output)
        # I'm not sure if this needs to be included or not, but maybe not?
        feed_forward_output = self.interm_af(feed_forward_output)
        # 4
        add_norm_output_second = self.add_norm(
            add_norm_output, feed_forward_output, self.out_dense, self.out_dropout, self.out_layer_norm)
        return add_norm_output_second
...
class BertModel(BertPreTrainedModel):
    ...
    def embed(self, input_ids):
        ...
        embed_layer_norm_output = self.embed_layer_norm(x)
        dropout_output = self.embed_dropout(embed_layer_norm_output)
