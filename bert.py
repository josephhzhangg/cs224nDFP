...
class BertSelfAttention(nn.Module):
  ...
  def attention(self, key, query, value, attention_mask):
    ...
    att_scores = torch.matmul(query, key.transpose(-2, -1)) * (1.0 / math.sqrt(key.size(-1)))
    bs, seq_len = att_scores.shape[0], att_scores.shape[2] #my guess from the instructions
    att_scores = att_scores.masked_fill_(attention_mask == 0, float('-inf'))
    att_scores = F.softmax(att_scores, dim=-1)
    V_prime = torch.matmul(att_scores, value)
    V_prime = V_prime.transpose(1, 2).contiguous().view(bs, seq_len, self.num_attention_heads * self.attention_head_size)
...
class BertLayer(nn.Module):
  ...
  def forward(self, hidden_states, attention_mask):
    ...
    #4
    add_norm_output_second = self.add_norm(add_norm_output, feed_forward_output, self.out_dense, self.out_dropout, self.out_layer_norm)
